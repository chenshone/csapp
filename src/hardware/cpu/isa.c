...
static void parse_instruction(const char *str, inst_t *inst, core_t *cr) {
    ...
    for (int i = 0; i < strlen(str); ++i) {
        ...
        if (state == 1)
            ...
            // od1
            src_str[src_str_len++] = c;
        else if (state == 5)
            // od2
            dst_str[dst_str_len++] = c;
    }
    ...
    debug_printf(DEBUG_PARSEINST, "[%s (%d)] [%s (%d)] [%s (%d)]\n", op_str, inst->op, src_str, inst->src.type, dst_str,
                 inst->dst.type);
}
...
static void add_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    if (src_od->type == REG && dst_od->type == REG) {
        ...
        // (src_sign == dst_sign == 0 && val_sign == 1) || (src_sign == 1 && dst_sign == 1 && val_sign == 0)
        cr->flags.OF = (!(src_sign ^ dst_sign) && (val_sign ^ src_sign));
...
static void sub_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    if (src_od->type == IMM && dst_od->type == REG) {
        ...
        // set condition flags
        cr->flags.CF = 0; // unsigned overflow
        cr->flags.ZF = (val == 0);
        cr->flags.SF = ((val >> 63) & 0x1);
        cr->flags.OF = 0; // singed overflow
